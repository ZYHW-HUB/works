...
def main():
    ...
    # 查询数据
    sql = "SELECT image_id, score FROM image_scores_yp WHERE split = 'lively'"
    cursor.execute(sql)
    rows = cursor.fetchall()
    
    # 打开CSV文件，写入表头及数据
    with open('walk/ArcGIS/lively/image_scores_lively.csv', 'w', newline='', encoding='utf-8') as csvfile:
