...
def load_pair_list(db_config):
    try:
        ...
        cursor = conn.cursor()
        query = "SELECT left_id, right_id, winner FROM pp2 WHERE category = 'lively'"
...
def create_scores_table(db_config):
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            CREATE TABLE IF NOT EXISTS image_scores_lively_S (
...
def insert_scores(db_config, scores, split, conn, cursor, batch_size=1000):
    try:
        query = "INSERT INTO image_scores_lively_S VALUES (%s, %s, %s) ON DUPLICATE KEY UPDATE score=VALUES(score)"
...
if __name__ == '__main__':
    ...
    # 日志与模型保存目录配置
    log_dir = "walk/deeplabv3-master/training_logs/siamese_lively"
