...
def load_pair_list(db_config):
    try:
        ...
        cursor = conn.cursor()
        query = "SELECT left_id, right_id, winner FROM pp2 WHERE category = 'safety'"  # 修改为对应的类别
...
def save_scores_to_db(db_config, scores, split='all'):
    try:
        ...
        cursor = conn.cursor()
        query = """
            INSERT INTO image_scores_safety_S 
            VALUES (%s, %s, %s)
...
if __name__ == '__main__':
    ...
    model_path = "walk/deeplabv3-master/training_logs/siamese_safety/saved_models/best_model.pth"  # 修改为实际路径
