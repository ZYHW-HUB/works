...
# 从数据库中读取评分数据
scores_query = "SELECT image_id, score FROM image_scores_beautiful_S"
...
# 保存相关系数和显著性到CSV文件
score_correlations.to_csv('walk/sequence/beautiful/correlation_matrix_beautiful.csv', index=True)
score_p_values.to_csv('walk/sequence/beautiful/p_values_matrix_beautiful.csv', index=True)

# 生成热力图
plt.figure(figsize=(10, 8))
sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', center=0, vmin=-1, vmax=1)
plt.title('Correlation Matrix Heatmap')
plt.savefig('walk/sequence/beautiful/correlation_matrix_heatmap_beautiful.png')
plt.show()
