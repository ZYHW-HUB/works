...
# 从数据库中读取评分数据
scores_query = "SELECT image_id, score FROM image_scores_boring_S"
...
# 保存相关系数和显著性到CSV文件
score_correlations.to_csv('walk/sequence/boring/correlation_matrix_boring.csv', index=True)
score_p_values.to_csv('walk/sequence/boring/p_values_matrix_boring.csv', index=True)

# 生成热力图
plt.figure(figsize=(10, 8))
sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', center=0, vmin=-1, vmax=1)
plt.title('Correlation Matrix Heatmap')
plt.savefig('walk/sequence/boring/correlation_matrix_heatmap_boring.png')
plt.show()
